...
if (!class_exists('SZGoogleActionRecaptchaLogin'))
{
	class SZGoogleActionRecaptchaLogin extends SZGoogleAction
	{
		...
		function addCaptchaLoginForm() 
		{
			...
			echo '<p><label for="recaptcha">'.SZGoogleCommon::getTranslate('reCAPTCHA','szgoogleadmin').'<br/>';
		...
		function addCaptchaLoginCheck($user,$username,$password)
		{
			...
			if (!$this->checkCaptca()) {
				$user = new WP_Error('denied','<strong>ERROR</strong>: '.ucfirst(__('check reCAPTCHA failed.','szgoogleadmin')));
