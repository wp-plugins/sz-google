...
function sz_google_admin_translate_callback() 
{
	...
	sz_google_common_form(
		ucfirst(__('google translate configuration','szgoogleadmin')),'sz_google_options_translate',$sections
	); 
}
...
function sz_google_admin_translate_mode() 
{
	...
	sz_google_common_form_select(
		'sz_google_options_translate','translate_mode',$values,'medium',''
	);

	sz_google_common_form_description(
		__('with this parameter you can set the type of view you want to use for the widget to translate the language selection, you can choose for example vertical, horizontal or simple. If you want to use a custom positioning can use the function <code>szgoogle_get_translate_code()</code>.','szgoogleadmin')
	);
}
...
function sz_google_admin_translate_language() 
{
	...
	sz_google_common_form_select(
		'sz_google_options_translate','translate_language',$values,'medium',''
	);
...
function sz_google_admin_translate_widget() 
{
	sz_google_common_form_checkbox_yesno(
		'sz_google_options_translate','translate_widget'
	);

	sz_google_common_form_description(
		__('if you enable this option you will find the widget required in the administration menu of your widget and you can plug it into any sidebar defined in your theme. If you disable this option, remember not to leave the widget connected to existing sidebar.','szgoogleadmin')
	);
}
...
{
	sz_google_common_form_checkbox_yesno(
		'sz_google_options_translate','translate_shortcode'
	);

	sz_google_common_form_description(
		__('if you enable this option you can use the shortcode [sz-gtranslate] and enter the corresponding component directly in your article or page. Normally shortcodes can be specified in the options, to control parameters given read the official documentation.','szgoogleadmin')
	);
}
...
{
	sz_google_common_form_checkbox_yesno(
		'sz_google_options_translate','translate_automatic'
	);

	sz_google_common_form_description(
		__('automatically display translation banner to users speaking languages other than the language of your page. If the language set on the visitor\'s browser is different from that of the website page displays the banner of translation. For details read the <a target="_blank" href="https://support.google.com/translate/">official documentation</a>.','szgoogleadmin')
	);
}
...
{
	sz_google_common_form_text(
		'sz_google_options_translate','translate_analytics_ua','medium',
		__('google analytics UA','szgoogleadmin')
	);

	sz_google_common_form_description(
		__('enter the code assigned to the profile of google analytics on which to collect statistical data relating to requests for translation. If you have the google analytics module of the plugin is automatically taken into the UA code shown in the specific module. Example UA-12345-12.','szgoogleadmin')
	);
}
