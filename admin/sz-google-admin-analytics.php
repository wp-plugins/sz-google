 ...
 */
if (!class_exists('SZGoogleModuleAdminAnalytics'))
{
	/**
	 ...
	 */
	class SZGoogleModuleAdminAnalytics extends SZGoogleModuleAdmin
	{
		/**
		...
		function __construct()
		{
			...
			if (is_admin() and $SZ_ANALYTICS_OPTION_ADMIN['ga_enable_admin'] == '1') 
			{
				if ($SZ_ANALYTICS_OPTION_ADMIN['ga_position'] == 'H') 
					add_action('admin_head'  ,'sz_google_module_analytics_add_script');

				if ($SZ_ANALYTICS_OPTION_ADMIN['ga_position'] == 'F') 
					add_action('admin_footer','sz_google_module_analytics_add_script');
			}
	 ...
	 */
	$SZ_GOOGLE_ADMIN_ANALYTICS = new SZGoogleModuleAdminAnalytics();
}